...
def test_compute_streaks_basic():
    ...
    entries = [
        HabitEntry(date=today.isoformat(), completed=True)
    ]
...
def test_compute_streaks_forgiving():
    ...
    entries = [
        HabitEntry(date=yesterday.isoformat(), completed=True)
    ]
    res = _compute_streaks(entries)
    assert res["currentStreak"] == 1
    assert res["bestStreak"] == 1
...
def test_compute_streaks_broken():
    ...
    entries = [
        HabitEntry(date=day_before_yesterday.isoformat(), completed=True)
    ]
    res = _compute_streaks(entries)
