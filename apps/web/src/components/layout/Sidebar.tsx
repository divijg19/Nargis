...
import { PlusIcon, ChevronLeftIcon, ChevronRightIcon, CalendarDaysIcon, PencilSquareIcon, ChartBarIcon, ClockIcon } from "@heroicons/react/24/outline";
...
export const Sidebar: React.FC<SidebarProps> = ({ mobileOpen = false, onMobileClose }) => {
    ...
    const examplePrompts: Prompt[] = [
        ...
        { title: "Begin a focus session", subtitle: "by saying 'start Pomodoro'", prompt: "Begin a focus session: start Pomodoro", icon: ClockIcon },
    ];
    ...
    const mobileOverlayClass = mobileOpen
        ? "fixed inset-0 z-40"
        : "hidden";
    ...
    return (
        <>
            ...
            >
                <div className="p-4 flex items-center">
                    <button
                        ...
                        type="button"
                        className={!collapsed ? "btn btn-primary px-3 py-2 text-sm w-full flex items-center gap-2 justify-start" : "p-2 rounded bg-primary text-primary-foreground"}
                ...
                <div className="px-4 py-2">
                    {!collapsed && <h3 className="text-sm font-semibold mb-2">Get Started</h3>}
                                ...
                                return (
                                    ...
                                    >
                                        {Icon ? (
                                            ...
                                            <svg className="w-5 h-5 text-muted-foreground" viewBox="0 0 24 24" fill="none" aria-hidden>
                                                <path d="M4 6h16M4 12h16M4 18h8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                            </svg>
                                        )}
                                    </button>
                                );
            ...
            <div className={mobileOverlayClass}>
                <div ref={overlayRef} onClick={onMobileClose} className="fixed inset-0 bg-black/40 z-40 md:hidden" />
                <button
                    ref={closeButtonRef}
                    type="button"
                ...
                />
                <aside ref={mobileAsideRef} role={mobileOpen ? "dialog" : undefined} aria-modal={mobileOpen ? true : undefined} aria-label="Sidebar" className={`fixed left-0 top-0 h-full z-50 w-72 pt-16 ${baseBg} border-r ${baseBorder} transform transition-transform duration-300 md:hidden ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
                    <div className="p-4">
                ...
                <>
                    ...
                    {mobileOpen && (
                        (() => {
                            ...
                            const focusFirst = () => {
                                const els = mAside?.querySelectorAll<HTMLElement>(focusableSelector) ?? [];
                            ...
                            const onKey = (e: KeyboardEvent) => {
                                ...
                                if (e.key === "Tab") {
                                    const els = Array.from(mAside?.querySelectorAll<HTMLElement>(focusableSelector) ?? []);
