...
export default function ChatPanel({ compact = false, merged = false, permissionDenied = false }: ChatPanelProps) {
	const {
		aiResponse,
		transcribedText,
		clearAiResponse,
					...
					<div className="mb-3">
						...
						<div className="flex items-center justify-between mb-3">
							<div className="flex items-center gap-3">
								{/* gradient accent */}
								<span className="w-1 h-8 rounded-full bg-gradient-brand inline-block" aria-hidden="true" />
								<div>
									<h3 id={headerId} className="text-sm font-semibold">AI Conversation</h3>
									<p className="text-xs text-muted-foreground">Live transcript & assistant reply</p>
								</div>
							</div>
							<div className="flex items-center gap-2">
								{/* Clear */}
								...
								<button
									...
									aria-label={embeddedOpen ? "Collapse conversation" : "Expand conversation"}
						...
						<div id="chat-merged-body" className={`grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3 collapse-transition ${embeddedOpen ? "expanded" : "collapsed"}`}>
							<div className="status-tile text-center">
								<div className="text-xs text-muted-foreground mb-1 font-medium">Speech Recognition</div>
								<div className={`text-sm font-semibold ${isListening ? "text-success" : "text-muted-foreground"}`}>
									{isListening ? "Active" : "Ready"}
								</div>
							</div>
							<div className="status-tile text-center">
								<div className="text-xs text-muted-foreground mb-1 font-medium">Transcription</div>
								<div className={`text-sm font-semibold ${transcribedText && !aiResponse ? "text-warning" : "text-muted-foreground"}`}>
									{transcribedText && !aiResponse ? "Processing" : "Standby"}
								</div>
							</div>
							<div className="status-tile text-center">
								<div className="text-xs text-muted-foreground mb-1 font-medium">AI Status</div>
						...
						{transcribedText && !aiResponse && (
							<div className="mt-1 p-3 bg-white/5 rounded-md border border-border/20 text-sm text-foreground/90 whitespace-pre-wrap" aria-live="polite">
						...
						{permissionDenied && (
							<div className="mt-3 p-3 glass bg-destructive/6 border border-destructive/20 rounded-lg">
								<div className="flex items-center space-x-3">
									...
									<div>
										<h3 className="text-sm font-semibold text-destructive">Microphone Access Required</h3>
										<p className="text-xs text-destructive/80 mt-1">Please enable microphone permissions to use voice features.</p>
									</div>
								</div>
							</div>
						)}
							...
							<div>
								...
								{!aiResponse ? (
									...
									<div>
										...
										<div className="max-h-44 overflow-auto text-sm leading-relaxed" aria-live="polite">
											{displayAi}
